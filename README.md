...
Run these after seeding to verify the important guarantees.
...
Salary records per company:

```sql
SELECT c.slug, COUNT(s.id) AS salary_count
FROM companies c
LEFT JOIN salaries s ON s.company_id = c.id
GROUP BY c.slug
ORDER BY salary_count DESC, c.slug ASC;
```

Edge cases:

```sql
SELECT id, role, level, bonus, stock, total_compensation
FROM salaries
WHERE bonus = 0 OR stock = 0 OR level = 'PRINCIPAL'
...
SELECT
  c.slug,
  percentile_cont(0.5) WITHIN GROUP (ORDER BY s.total_compensation) AS median_total_compensation
FROM companies c
JOIN salaries s ON s.company_id = c.id
GROUP BY c.slug
...
FROM pg_indexes
WHERE tablename IN ('companies', 'salaries')
